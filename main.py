...
def crawl(url: dict, links: dict, path: list[str], on_path: bool, depth: int, 
    ...
    for topic in selected:
        links[topic] = {}
        crawl(topic_url(topic), links[topic], path, on_path and topic == next_topic, depth-1, width, bar)
    ...
    def view(self, clear=True):
        ...
        if len(self.chosen_path) == len(self.path) - 1:
            ...
            if final_path == self.path:
                ...
                colour = Fore.RED
            string = colour + '    '.join([topic for topic in final_path]) + Fore.WHITE + '\n' * (self.width)
        else:
            # Init rows
            rows = [['' for _ in range(self.depth+2)] for _ in range(self.width)]
